...
.wuji-container.center-block
  .wuji-category(style="text-align:left")
    el-button(@click="$router.push({ path: 'Diary' })")
      i.el-icon-back
    el-button.add-btn(@click="handleAdd", type="primary",style="float: right") 添加我的分类
    el-table(:data="categoryList", style="width: 100%, margin-bottom:5px")
    ...
    el-dialog(:title="title", :visible.sync="dialogFormVisible" )
      el-form(:model="form" style="text-align: left")
        el-form-item
          el-input(v-model="form.id", type="hidden", auto-complete="off")
  ...
  methods: {
    ...
    doSave() {
      ...
      if (errorMsg) {
        ...
        (this.categoryList.some(this.isSameName) ||
          this.categoryList.some(this.isSameColorHex)) &&
        this.isCreate
      ) {
        return Message({
          message: '分类名称和颜色不能有相同的！',
          type: 'error',
          duration: 2000,
        });
      } else if (!this.isCreate) {
        const filterList = this.categoryList.filter(
          item => item.id !== _self.editRow.id,
        );
